...
{
    public partial class EditConteudo : System.Web.UI.Page
    {
        ...
        public List<Beacon> GetBeacons(string email)
        {
            HttpClient client = new HttpClient();
            try
            {
                ...
                return beacons.Where(b => b.Entidade.email.Equals(email)).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public void Save_Click(object sender, EventArgs e)
        {
            ...
            int _id = Int32.Parse(Request.QueryString["ConteudoID"]);
            Conteudo cont = GetConteudo(_id);
            Response.Redirect("AddEntidade?EntidadeID="+cont.Beacon.entidade_id);
        }
        public void Cancel_Click(object sender, EventArgs e)
        {
            int id = Int32.Parse(Request.QueryString["ConteudoID"]);
            Conteudo cont = GetConteudo(id);
            Response.Redirect("AddEntidade?EntidadeID="+ cont.Beacon.entidade_id);
        }
    }
}